...
7. Phase 3：前端管理台与分析
   ...
   - 刷新管理台 UI 为克制后台仪表盘风格，移除旧紫色霓虹主导视觉，改用哑光深色底、绿色/青色状态色和第一屏指挥中心。
   - 为活动表/日志表增加独立表格类、列宽策略、长 URL 单行省略和真实浏览器触控目标回归。
   - 修复 opt-in 浏览器布局测试的认证准备步骤，使临时 Chrome 写入管理令牌后再等待数据状态。
   - 将管理台从单页堆叠重构为总览、活动、访问日志、系统设置四个视图，降低单屏信息密度并保留现有功能入口。
   - 增加 hash 视图切换和顶部标题同步，浏览器布局测试分别覆盖四个视图。
8. Phase 4：生产部署与使用手册
   - 新增生产部署设计 spec 与执行计划。
   ...
   - 新增 Bot IP 文件同步 CLI：`npm run bot-ips:sync`，支持外部文本 URL / 本地文本文件、dry-run、去重输出。
   - 新增生产监控 CLI：`npm run monitor:production`，检查 `/health` 与 `/api/v1/settings`。
   - 监控 CLI 支持 `--alert-webhook-url`，失败时发送通用 JSON Webhook 告警。
   - 监控 CLI 支持可选 Analytics 阈值检查：`--check-analytics`、`--max-bot-percent`、`--max-suspicious-percent`。
...
- 若未来需要完整公网生产发布，可继续补自动部署流水线；本轮按用户说明不处理真实服务器部署。
- 若要进一步增强真实机器人识别，可接入 Redis/数据库 Bot IP source 管理界面；当前文件型名单已支持外部文本同步和后台重载。
- 若要上线公网，备份/恢复脚本、恢复演练入口、文件型 Bot IP 同步/重载、健康/设置告警和 Analytics 阈值告警入口已具备；仍可按实际基础设施补自动部署。
