...
`npm run monitor:production` 会检查运行中应用的 `/health` 与 `/api/v1/settings`，失败时可通过 `--alert-webhook-url` 发送 JSON 告警；追加 `--check-analytics --max-bot-percent 80 --max-suspicious-percent 20` 后会同时检查 Analytics 判定占比。
...
node src/database/run-postgres-api-smoke-check.js --database-url postgres://cloak:secret@127.0.0.1:5432/cloak --check-health
